...
I mainly use AI as a suggestion and not let it do everything. The Ai tends to prefer adding new functions sometimes and for some cases it does work while others it was uneccesary. I first ask it about the logic of certain parts of the code, especially functions, to see whether the AI could understand it. It did that pretty well, being able to also discover some logic errors that I didn't prompt it about. I usually nudge it on how it should create its fixes whenever it prompts possible fixes. I'm reading each fix to make sure the AI isn't doing something uneccesary or edit too much. WHenever it makes a test whenever its for something simple it makes good tests. However its better if the code you want to test is in its own function, as any code isn't in a function its not going to give me a test that works, but it will give me something anyway. I didn't let it make any large edits and prefer to only look at it case by case, if it understood the logic behind some parts of the code.
I like using Claude to review my code in case of any syntax or any semanantics of any library code I may not fully understand. It seems to be great for small bursts and for a second opinion when one is unsure about something.